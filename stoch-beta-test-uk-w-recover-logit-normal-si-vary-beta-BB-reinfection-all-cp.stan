...
transformed parameters {
 ...
 real kappa = 1 / inv_kappa;
  ...
  r_t[1] = 0;
  for (n in 2:TT) {
    u_t[n] = inv_logit(u_t_logit[n]);
    w_t[n] = inv_logit(w_t_logit[n]);
    if (r_t[n-1] > 0)
      v_t[n] = inv_logit(v_t_logit[n]);
    else 
      v_t[n] = 0;
...
model {
 ...
 u_t_logit[1] ~ std_normal();
 w_t_logit[1] ~ std_normal();
 v_t_logit[1] ~ std_normal();
  for (n in 2:TT) {
    int n_idx = (n - 1) %/% b_freq + 1;
    real u_t_mean = exponential_cdf(b[n_idx] * i_t[n - 1] | 1);
    u_t_logit[n] ~ normal(logit(u_t_mean),sqrt(kappa + pop_size * s_t[n-1]) / sqrt(pop_size * s_t[n-1] * u_t_mean * (1 - u_t_mean) * (kappa + 1)));
    w_t_logit[n] ~ normal(logit(gamma), 1 / sqrt(pop_size * i_t[n-1] * gamma * (1 - gamma)));
    if (r_t[n-1] > 0)
      v_t_logit[n] ~ normal(logit(phi), 1 / sqrt(pop_size * r_t[n-1] * phi * (1 - phi)));
    else
      v_t_logit[n] ~ std_normal();
  }
  for (i in 2:TT) {
