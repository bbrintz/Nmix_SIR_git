...
parameters {
  ...
  array[N_seg] vector[N_C] log_beta_diag; // Diagonal elements of log_beta
}
...
transformed parameters {
  ...
  array[N_seg] matrix[N_C, N_C] log_beta;
  //matrix[N_C, N_C] log_beta;
  array[N_seg] matrix[N_C, N_C] beta;
  ...
  log_beta[n] = rep_matrix(0, N_C, N_C);

  // Set diagonal elements
  for (i in 1:N_C) {
    log_beta[n][i, i] = log_beta_diag[n][i];
  }
  ...
  for (i in 1:(N_C - 1)) {
    for (j in (i + 1):N_C) {
      ...
      log_beta[n][i, j] = log_beta[n][i, i] - decay_rate_space * D[i, j];
      log_beta[n][j, i] = log_beta[n][i, j];
    }
  }
  }
  ...
  for (n in 2:TT) {
    ...
    for (ct in 1:N_C) {
      ...
      real u_t_mean = exponential_cdf(dot_product(beta[n_idx][ct,], i_t[n - 1,]) | 1);
...
model {
 ...
 for (i in 1:(TT-1)) {
  int n_idx = (i - 1) %/% b_freq + 1;
  log_beta_diag[n_idx] ~ multi_normal(rep_vector(0, N_C), Sigma);
  for (ct in 1:N_C) {
        if (se_t[i+1,ct] > 0)
